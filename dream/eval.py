...
class Dream(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.temporal_eval = temporal_eval
        self.confidence_cluster_size = confidence_cluster_size
        self.spatial_threshold = spatial_threshold
        self.confidence_cluster_unmasked = confidence_cluster_unmasked
        self.token_cluster = token_cluster
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        generation_ids = self.model.diffusion_generate(
            ...
            temporal_eval=self.temporal_eval,
            confidence_cluster_size=self.confidence_cluster_size,
            spatial_threshold=self.spatial_threshold,
            confidence_cluster_unmasked=self.confidence_cluster_unmasked,
            token_cluster=self.token_cluster,
            token_cluster_size=self.token_cluster_size,
        )
        ...
        if hasattr(generation_ids, 'history') and generation_ids.history is not None:
            self.last_generation_history = generation_ids.history
        responses = [
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            if hasattr(self, 'last_generation_history') and self.last_generation_history:
                for i, r in enumerate(responses):
                    metadata = {"context": contexts[i], "response": r}
                    for hist in self.last_generation_history:
                        if 'total_denoising_steps' in hist:
                            metadata["total_denoising_steps"] = hist['total_denoising_steps']
                        if 'num_blocks' in hist:
                            metadata["num_blocks"] = hist['num_blocks']
                        if 'steps_per_block' in hist:
                            metadata["steps_per_block"] = hist['steps_per_block']
                        if 'algorithm' in hist:
                            metadata["algorithm"] = hist['algorithm']
                        if 'avg_main_steps_per_block' in hist:
                            metadata["avg_main_steps_per_block"] = hist['avg_main_steps_per_block']
                        if 'max_gpu_mem_gb' in hist:
                            metadata["max_gpu_mem_gb"] = hist['max_gpu_mem_gb']
                        if 'avg_gpu_mem_gb' in hist:
                            metadata["avg_gpu_mem_gb"] = hist['avg_gpu_mem_gb']
                        if 'remask_token_num' in hist:
                            metadata["remask_token_num"] = hist['remask_token_num']
        ...
        speed_logs = [
            ...
            f"average latency per sample: {elapsed / len(requests)} seconds",
            f"Generated token num: {self.generated_token_num}",
            f"Generated token num per second: {self.generated_token_num / elapsed}",
        ]
        if all_metadata:
            samples_with_steps = [m for m in all_metadata if 'total_denoising_steps' in m]
            if samples_with_steps:
                total_steps = [m.get('total_denoising_steps', 0) for m in samples_with_steps]
                avg_total_steps = sum(total_steps) / len(total_steps) if total_steps else 0
                total_remask_num = [m.get('remask_token_num', 0) for m in samples_with_steps]
                avg_remask_num = sum(total_remask_num) / len(total_remask_num) if total_steps else 0
                total_max_mems = [m.get('max_gpu_mem_gb', 0) for m in samples_with_steps]
                avg_max_mem = sum(total_max_mems) / len(total_max_mems) if total_steps else 0
                total_ave_mems = [m.get('avg_gpu_mem_gb', 0) for m in samples_with_steps]
                avg_avg_mem = sum(total_ave_mems) / len(total_ave_mems) if total_steps else 0
                speed_logs.extend(
                    [
                        f"Avg total denoising steps: {avg_total_steps:.2f}",
                        f"Avg remask token num: {avg_remask_num:.2f}",
                        f"Avg maximum GPU memory: {avg_max_mem:.2f}",
                        f"Avg averaged GPU memory: {avg_avg_mem:.2f}",
                    ]
                )

        for line in speed_logs:
            print(line)

        if self.save_dir is not None:
            rank = self.rank
            metrics_path = os.path.join(self.save_dir, f"rank_{rank}_speed.txt")
            with open(metrics_path, "a", encoding="utf-8") as f_speed:
                f_speed.write("\n".join(speed_logs) + "\n")
