...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.record_decode = bool(record_decode)
        self.decode_events_dir = decode_events_dir
    @property
    def rank(self):
        return self._rank
    ...
    def generate_until(self, requests):
        ...
        if self.save_dir is not None:
            ...
            save_tokens_path = os.path.join(self.save_dir, f'rank_{rank}_tokens.jsonl')
        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe = generate_with_dual_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        ...
                                        record_decode=self.record_decode,
                                        decode_events_dir=self.decode_events_dir)
                else:
            ...
            if self.save_dir is not None:
                ...
                if save_tokens_path is not None:
                    with open(save_tokens_path, 'a', encoding='utf-8') as f_tok:
                        for idx, generated_answer in enumerate(batched_generated_answer):
                            ...
                            for pos, tid in enumerate(token_ids):
                                tok = token_strs[pos] if pos < len(token_strs) else None
                            ...
                            payload = {
                                "text_index": base_index + idx,
                                "tokens": token_records,
                            }
                            f_tok.write(json.dumps(payload, ensure_ascii=False) + '\n')
        ...
        if self.show_speed:
            ...
            speed_logs = [
                f"Total number of tokens generated: {num_tokens}",
                f"Total time taken: {elapsed} seconds",
                f"Tokens per second: {num_tokens / elapsed}",
                f"Total NFE is {num_nfe}",
                f"Average NFE is {num_nfe / len(requests)}",
            ]
